...
orderRouter.get("/mine", isAuthenticated, async (req, res) => {
...
orderRouter.delete("/:id", isAuthenticated, isAdmin, async (req, res) => {
  const order = await Order.findOne({ _id: req.params.id });
  if (order) {
...
orderRouter.put("/:id/pay", isAuthenticated, async (req, res) => {
